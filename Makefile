...
debug_tests: build/run_tests
	lldb build/run_tests


build/test_btee.o: $(OBJECTS) tests/test_btree.cc include/inverted_index.h include/phy.h build/libgtest.a
build/run_tests: tests/*.cc $(OBJECTS) include/inverted_index.h build/libgtest.a
	$(CXX) $(CXX_FLAGS) -g -o build/run_tests \
		-pthread \
		-isystem ${GTEST_DIR}/include \
		-Iinclude \
		build/libgtest.a $(OBJECTS) tests/*.cc
...
build/phy/%.o: src/phy/%.cc include/inverted_index.h include/phy.h | build
